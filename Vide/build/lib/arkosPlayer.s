...
tmp_track_param: .blkb   1
arkosPlayerMemStart  = tmp_track_param
tmp_track_instrument: .blkb   1
tmp_instrument_second_byte: .blkb   1
; following are player vars which are channel independend
PLY_HEIGHT: .blkb   1                            ; height of pattern 
PLY_SPEED: .blkb   1                            ; speed of pattern 
PLY_SPEEDCPT: .blkb   1                            ; current speed position (count down to 0) 
PLY_HEIGHTCPT: .blkb   1                            ; current height position (count down to 0) 
PLY_TRACK_INSTRUMENTSTABLEPT: .blkb   2                       ; address of instrument table 
PLY_LINKER_PT: .blkb   2                            ; current linker position 
PLY_PSGREG13_RETRIG: .blkb   1                            ; retrigger "flag" - if same as PLY_PSGREG13, than not retriggered, otherewise - yes 
PLY_SAVESPECIALTRACK: .blkb   2                            ; start position of current special track 
PLY_SPECIALTRACK_PT: .blkb   2                            ; current position in special track 
PLY_SPECIALTRACK_WAITCOUNTER: .blkb   1                       ; wait counter for special track (count down to 0) 
;
; in general in belo player, y reg points to the start of
; following structure (one for each channel)
;                    struct   ArkosChannel 
                    PLY_TRANSPOSITION = 0 ;, 1 
                    PLY_TRACK_SAVEPTINSTRUMENT = 1
                    PLY_TRACK_INSTRUMENT = 3
                    PLY_TRACK_INSTRUMENTSPEED = 5
                    PLY_TRACK_INSTRUMENTSPEEDCPT = 6
                    PLY_TRACK_PT = 7
                    PLY_TRACK_WAITCOUNTER = 9
					PLY_TRACK_NOTE = 10
                    PLY_TRACK_VOLUME = 11
                    PLY_TRACK_PITCH = 12
                    PLY_TRACK_PITCHADD = 14
                    PLY_TRACK_REG_7 = 16
;                    end struct 
ArkosChannel = 17

;
Channel1Data:        .blkb   17 ;ds       ArkosChannel 
Channel2Data:        .blkb   17 ;ds       ArkosChannel 
Channel3Data:        .blkb   17 ;ds       ArkosChannel 
ChannelDataEnd      = Channel3Data + 17
;
PLY_PSGREG13:        .blkb   1 ; ds       1                            ; special - this is used for retrigger activities 
PLY_VOL_REG:         .blkb   2 ; ds       2                            ; these two are used to stay "channel" independend 
PLY_FREQ_REG:        .blkb   2 ; ds       2                            ; they are loaded befor the "work" routines with the corresponding regs of the current channel 

;	org      Vec_Music_Work 
; simple redefines for source compatability (TODO: Vec_Music_Work is 0hc83f, so can't be redefines)
	.globl PLY_PSGREG0
PLY_PSGREG0:         .blkb   1
PLY_PSGREG1:         .blkb   1
PLY_PSGREG2:         .blkb   1
...
PLY_PSGREG11:        .blkb   1
PLY_PSGREG12:        .blkb   2
PLY_PSGREGISTERSARRAY = PLY_PSGREG0
PLY_PSGREGISTERSARRAY_END = PLY_PSGREG12 + 2

arkosPlayerMemEnd = PLY_PSGREG12 + 2


	.area .text

PLY_RETRIGVALUE     = 0hFE                         ; some value greater than 0x0f 
PLAYER: 
	.globl PLY_PLAY
PLY_PLAY: 
...
PLY_PT_SPECIALTRACK_ENDDATA: 
                    LDA      #0h1                          ; reset wait counter, next round might be more waiting :-) 
...
PLY_READTRACK_READPARAMETERS: 
                    ...
                    bita     #0h80                         ; is pitch following? -> load it 
                    beq      PLY_READTRACK_PITCH_END 
                    ldu      ,x++ 
                    stu      PLY_TRACK_PITCHADD,y 
PLY_READTRACK_PITCH_END: 
                    bita     #0h20                         ; is instrument following? -> load it 
...
;No Wait command. Can be a Note and/or Effects.
                    lda      tmp_track_param 
                    STX      PLY_TRACK_PT, y 
                    bita     #0h40                         ;Note ? If no Note, we don't have to test if a new Instrument is here. 
...
full_opt_note_given: 
                    ...
                    LDX      #0h0                          ;Reset the TrackPitch. 
                    STX      PLY_TRACK_PITCH , y 
                    bita     #0h20                         ;New Instrument ?; 
...
;n = Second Byte needed? Needed when Noise, or Manual frequency, or noise with no sound.
                    ...
                    lda      #9                           ; these are the register mask bits for this sound (or this no sound) 
...
PLY_PS_HARD_NORETRIG: 
                    ...
                    bitb     #2                          ;WE DON'T SHIFT THE BITS, SO THAT WE CAN USE THE SAME CODE (FREQUENCY CALCULATION) SEVERAL TIMES. 
...
; shift is stored in inverse, 7 - shift
                    ...
                    LSRb     
                    LSRb     
                    ANDb     #14                         ; blend out all other data 
                    clra     
...
PLY_PS_S_SOUNDON_ARPEGGIOEND: 
                    ...
                    LSLB                                  ; for pointer in table double it 
                    ROLA     
...
;Store the Linker address.
                    STu      PLY_LINKER_PT 
                    lda      #0hff                         ; make sure the hardware envelope is in an "unkown" state 
                    STA      PLY_PSGREG13 
